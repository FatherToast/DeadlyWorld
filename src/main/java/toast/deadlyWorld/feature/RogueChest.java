...
public class RogueChest implements WorldFeature {
    ...
    public static final int[] weights = WorldGenerator.getWeights(Properties.CHESTS, _DeadlyWorld.CHESTS);
    ...
    public void place(World world, Random random, int x, int y, int z) {
        String type = WorldGenerator.choose(random, RogueChest.totalWeight, _DeadlyWorld.CHESTS, RogueChest.weights);
        if (type == "valuable") {
    ...
    private boolean indieTrapChest(World world, Random random, int x, int y, int z, byte dir) {
        if (!this.canPlaceTNT(world, x += Direction.offsetX[dir] << 1, y, z += Direction.offsetZ[dir] << 1) || world.getBlock(x, y, z).hasTileEntity(world.getBlockMetadata(x, y, z)) || !this.canPlaceTNT(world, x += Direction.offsetX[dir], y, z += Direction.offsetZ[dir]))
            return false;
