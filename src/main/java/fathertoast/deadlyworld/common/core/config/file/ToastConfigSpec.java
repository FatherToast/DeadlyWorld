package fathertoast.deadlyworld.common.core.config.file;
...
public class ToastConfigSpec {
    ...
    public ToastConfigSpec( File dir, String fileName ) {
        ...
        if( !dir.exists() && !dir.mkdirs() ) {
            DeadlyWorld.LOG.error( "Failed to make config folder! Things will likely explode. " +
    ...
    public void initialize() {
        DeadlyWorld.LOG.info( "First-time loading config file {}", CONFIG_FILE.getFile() );
        firstLoad = true;
        CONFIG_FILE.load();
    }
    ...
    private static class Header extends Format {
        ...
        @Override
        public void write( ToastTomlWriter writer, CharacterOutput output ) {
            writer.writeComment( DeadlyWorld.MOD_ID + ":" + PARENT.NAME + ToastConfigFormat.FILE_EXT, output );
