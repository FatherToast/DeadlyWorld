...
public class BlockEntry implements Cloneable {
    ...
    public BlockEntry( AbstractConfigField field, String line ) {
        ...
        if( !pair[1].endsWith( "]" ) ) {
            ...
            DeadlyWorld.LOG.info( "Adding closing bracket on block state properties for {} \"{}\". Invalid entry: {}",
    ...
    public String toString() {
        // Block name
        String registryName = DeadlyWorld.toString( BLOCK );
        if( MATCHERS.isEmpty() ) {
            return registryName;
        }
        // Append block states
    ...
    void mergeFrom( BlockEntry other ) {
        if( MATCHERS.isEmpty() ) {
            DeadlyWorld.LOG.warn( "Ignoring attempt to add redundant block state to config with block state wildcard '{}'", other );
        }
        else if( other.MATCHERS.isEmpty() ) {
            DeadlyWorld.LOG.warn( "Adding block state wildcard to config with redundant block state(s) '{}'", this );
            MATCHERS.clear();
        }
        else {
    ...
    private static State parseState( AbstractConfigField field, String stateString, Block block ) {
        ...
        for( String combinedEntry : properties ) {
            ...
            if( pair.length != 2 ) {
                DeadlyWorld.LOG.warn( "Invalid block property for {} \"{}\". Format must be 'property=value'. " +
            ...
            if( property == null ) {
                ...
                DeadlyWorld.LOG.warn( "Invalid block property key for {} \"{}\". Valid property keys for '{}' are {}. " +
                                "Deleting property. Invalid property: {}", field.getClass(), field.getKey(), DeadlyWorld.toString( block ),
            ...
            if( !value.isPresent() ) {
                ...
                DeadlyWorld.LOG.warn( "Invalid block property value for {} \"{}\". Valid values for property '{}' are {}. " +
