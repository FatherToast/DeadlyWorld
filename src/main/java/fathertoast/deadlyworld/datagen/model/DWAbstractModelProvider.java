...
public abstract class DWAbstractModelProvider extends BlockStateProvider {
    ...
    protected void spikeTrap( RegistryObject<? extends BaseSpikeTrapBlock> regObj,
                              ResourceLocation baseTexture, ResourceLocation spikesTexture, ResourceLocation overlay ) {
        final String name = Objects.requireNonNull( regObj.getId() ).getPath();
        final Block block = regObj.get();
    ...
    protected void spikeTrap( RegistryObject<? extends BaseSpikeTrapBlock> regObj, ResourceLocation baseTexture ) {
        SpikeTrapType type = regObj.get().getSpikeTrapType();
        
        if( regObj.get() instanceof MechanicalSpikeTrapBlock ) {
            ResourceLocation spikesTexture = modBlockTexture( type.toString().replaceFirst( "mechanical_", "" ) + "_spikes" );
            mechanicalSpikeTrap( regObj, baseTexture, spikesTexture, modBlockTexture( "regular_spike_trap_overlay" ) );
        }
        else {
            ResourceLocation spikesTexture = modBlockTexture( type + "_spikes" );
            spikeTrap( regObj, baseTexture, spikesTexture, modBlockTexture( "regular_spike_trap_overlay" ) );
        }
    }
